...
export const useLayoutMenu = defineStore("layout-menu", () => {
  ...
  const handleAccordionMode = (innerOpenKeys: string[]) => {
    ...
    const intersection = innerOpenKeys.filter((value) =>
      openKeys.value?.includes(value)
    );
    const currentClickKey = innerOpenKeys
      .concat(openKeys.value)
      .filter((value) => !intersection.includes(value))[0];
    ...
    if (rootSubmenuKeys?.includes(currentClickKey)) {
      ...
      // if (
      //   rootSubmenuKeys.filter((value) => openKeys.value.includes(value))
      //     .length === 0
      // )
      //   openKeys.value = [];
      // else openKeys.value = innerOpenKeys;
      switch (innerOpenKeys.includes(currentClickKey)) {
        case true:
          openKeys.value = innerOpenKeys;
          break;
        case false:
          openKeys.value = innerOpenKeys.slice(
            0,
            openKeys.value.indexOf(currentClickKey)
          );
          break;
      }
    }
