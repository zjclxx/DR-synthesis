          ...
          <template #actions>
            <template v-if="item.key === 'contentWidth'">
              <a-select
                ...
                @update:value="(e:string) => handleChangeSetting('contentWidth', e)">
                <a-select-option v-if="layout === 'top'" value="Fixed">
                  Fixed
                </a-select-option>
                <a-select-option value="Fluid"> Fluid </a-select-option>
              </a-select>
            </template>
            <template v-if="item.key === 'fixedHeader'">
              <a-switch
                ...
                :checked="fixedHeader"
                :disabled="item.disabled"
                @update:checked="(e:boolean) => handleChangeSetting('fixedHeader', e)" />
            </template>
            <template v-if="item.key === 'fixSiderbar'">
              <a-switch
                ...
                :checked="fixedSider"
                :disabled="item.disabled"
                @update:checked="(e:boolean) => handleChangeSetting('fixedSider', e)" />
            </template>
            <template v-if="item.key === 'splitMenus'">
              <a-switch
                ...
                :checked="splitMenus"
                :disabled="item.disabled"
                @update:checked="(e:boolean) => handleChangeSetting('splitMenus', e)" />
            </template>
            <template v-if="item.key === 'accordionMode'">
              <a-switch
                ...
                :disabled="item.disabled"
                @update:checked="
                  (e: boolean) => handleChangeSetting('accordionMode', e)
                " />
            </template>
          </template>
